...
public class DTOGenerator extends ClassGenerator{
    ...
    public StringBuffer buildClass(String className, ConcurrentHashMap<String,TableData> attributes, String pkgName) throws Exception {
        if(!className.isEmpty() && attributes.size()>0 && !pkgName.isEmpty() ){
            ...
            stringBuffer.append("\n");
            stringBuffer.append("public class "+Util.toTitleCase(className) +" implements Serializable {");
            stringBuffer.append("\n");
            String pkId = null;
            String fkId = null;
            ...
            for(Object key : attributes.keySet()) {
                ...
                if(tableData.getFk()){
                    if(!tableData.getDone() && tableData.getLeft()){
                        stringBuffer.append("\n");
                        stringBuffer.append("\t");
                        stringBuffer.append("@OneToOne(mappedBy = '"+ className+"', fetch = FetchType.LAZY, cascade = CascadeType.ALL)");
                        stringBuffer.append("\n");
                        stringBuffer.append("\t");
                        stringBuffer.append("private "+ Util.toTitleCase(tableData.getJoinedTable())+" "+lowerCase(tableData.getJoinedTable())+";");
                        stringBuffer.append("\n");
    ...
    public Boolean writeClass(String className, String classStr, String fileLocation,String packageName) throws IOException  {
        ...
        if (!packageName.isEmpty() && !fileLocation.isEmpty()) {
            ...
            try {
                ...
                File file = new File(fileLocation + "/" + filePath + "/" + Util.toTitleCase(className)  + ".java");
              //  file.deleteOnExit();
                file.createNewFile();
